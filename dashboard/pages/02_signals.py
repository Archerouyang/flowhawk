...
# --- Run pipeline ---
if run_pipeline:
    with st.spinner("Running anomaly detection → technical filter → LEAPS selection..."):
    ...
    if leaps_df.is_empty():
        st.warning("No LEAPS signals generated. Try expanding the symbol universe or relaxing filters.")
    else:
        ...
        for i in range(top_n):
            ...
            with st.container():
                ...
                with col_l:
                    st.plotly_chart(fig, use_container_width=True, key=f"greeks_radar_{i}")
                ...
                with col_r:
                    ...
                    rationale_parts = []
                    if row["voi_ratio"] > 3:
                        rationale_parts.append(f"✅ Volume/OI spike ({row['voi_ratio']:.1f}x)")
                    if row["theta_price_ratio"] < 0.003:
                        rationale_parts.append(f"✅ Low time decay ({row['theta_price_ratio']:.2%})")
                    ...
                    risk_cols[1].metric("Breakeven", f"${row['strike'] + row['last_price']:.2f}" if row["option_type"] == "C" else f"${row['strike'] - row['last_price']:.2f}")
        ...
        display_cols = [
            "symbol", "option_type", "strike", "expiration", "last_price",
            "delta", "implied_volatility", "voi_ratio", "leaps_score",
        ]
